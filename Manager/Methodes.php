...
public function modif_reservation(reservation $modif_reservation){
  ...
  $rec = $bdd->prepare('SELECT nom FROM reservation WHERE reservation=?');
  $rec->execute(array($modif_reservation->getReservation()));
  $nom = $rec->fetch();
  ...
  $reserv = $nom[0]."/".$modif_reservation->getFilm()."/".$modif_reservation->getDate();
